...
;(function (global) {
  ...
  var _data_type_size = new Int32Array([
  ...
  var matrix_t = (function () {
    ...
    matrix_t.prototype.allocate = function () {
      ...
      delete this.buffer
      //
      this.buffer = new data_t(
        this.cols * get_data_type_size(this.type) * this.channel * this.rows,
      )
      this.data =
        this.type & U8_t
          ? this.buffer.u8
    ...
    return {
      ...
      perspective_4point_transform: function (
        ...
        var Hr0 =
          ...
          t30
        var Hr1 =
          (t5 - t8 - t32 * t4 + t32 * t9 + t18 - t2 * t35 + t27 * t2 - t20) *
          t30
        var Hr2 = t1
        var Hr3 =
  ...
  var linalg = (function () {
    ...
    var JacobiImpl = function (A, astep, W, V, vstep, n) {
      ...
      for (k = 0; k < n; k++) {
        ...
        if (k < n - 1) {
          for (
            m = k + 1, mv = Math.abs(A[astep * k + m]), i = k + 2;
            i < n;
            i++
          ) {
      ...
      if (n > 1)
        for (; iters < max_iter; iters++) {
          ...
          for (j = 0; j < 2; j++) {
            ...
            if (idx < n - 1) {
              for (
                m = idx + 1, mv = Math.abs(A[astep * idx + m]), i = idx + 2;
                i < n;
                i++
              ) {
...
;(function (global) {
      ...
      for (; j < i; ++j) {
        ...
        for (k = 0; k < j; ++k) {
          ...
          if (
            ...
            jsfeat.EPSILON *
              (Math.abs(dx1) + Math.abs(dy1) + Math.abs(dx2) + Math.abs(dy2))
          )
            return true
        }
      }
    ...
    var affine2d = (function () {
      ...
      affine2d.prototype.error = function (from, to, model, err, count) {
        ...
        for (; i < count; ++i) {
          ...
          err[i] =
            sqr(pt1.x - m[0] * pt0.x - m[1] * pt0.y - m[2]) +
    ...
    var homography2d = (function () {
      ...
      homography2d.prototype.check_subset = function (from, to, count) {
        ...
        if (count == 4) {
          ...
          var detA = jsfeat.matmath.determinant_3x3(
            A11,
            A12,
            A13,
            A21,
          ...
          var detB = jsfeat.matmath.determinant_3x3(
            B11,
            B12,
            B13,
            B21,
          ...
          detA = jsfeat.matmath.determinant_3x3(
            A11,
            A12,
            A13,
            A21,
          ...
          detB = jsfeat.matmath.determinant_3x3(
            B11,
            B12,
            B13,
            B21,
  ...
  var ransac_params_t = (function () {
    ...
    ransac_params_t.prototype.update_iters = function (_eps, max_iters) {
      ...
      return (
        (denom >= 0 || -num >= max_iters * -denom
          ? max_iters
          : Math.round(num / denom)) | 0
      )
    }
  ...
  var motion_estimator = (function () {
    ...
    return {
      ransac: function (
        ...
        var curr_mask = new jsfeat.matrix_t(
          count,
          1,
          jsfeat.U8C1_t,
          ms_buff.data,
      ...
      lmeds: function (
        ...
        if (result) {
          sigma =
            2.5 *
            1.4826 *
            (1 + 5.0 / (count - model_points)) *
            Math.sqrt(min_median)
      ...
      for (sy = 0; sy < h; sy++) {
        ...
        if ((dy + 1) * scale_y <= sy + 1 || sy == h - 1) {
          ...
          if (beta <= 0) {
            for (dx = 0; dx < nw * ch; dx++) {
              dst_d[b + dx] = Math.min(
                Math.max((sum[dx] + buf[dx] * 256) / inv_scale_256, 0),
                255,
              )
              sum[dx] = buf[dx] = 0
            }
            ...
            for (dx = 0; dx < nw * ch; dx++) {
              dst_d[b + dx] = Math.min(
                Math.max((sum[dx] + buf[dx] * beta1) / inv_scale_256, 0),
                255,
              )
              sum[dx] = buf[dx] * beta
              buf[dx] = 0
    ...
    return {
          ...
          for (
            ...
            dst_u8[jr + 2] =
              (src[ir + cn2] * coeff_r +
                ...
                src[ir + cn2 + 2] * coeff_b +
                8192) >>
              14
            dst_u8[jr + 3] =
              (src[ir + cn3] * coeff_r +
                src[ir + cn3 + 1] * coeff_g +
                src[ir + cn3 + 2] * coeff_b +
                8192) >>
              14
          }
          for (; x < w; ++x, ++jr, ir += cn) {
      ...
      resample: function (src, dst, nw, nh) {
        ...
        if (h > nh && w > nw) {
          ...
          if (
            src.type & jsfeat.U8_t &&
            dst.type & jsfeat.U8_t &&
            (h * w) / (nh * nw) < 0x100
          ) {
      ...
      box_blur_gray: function (src, dst, radius, options) {
        ...
        var scale =
          options & jsfeat.BOX_BLUR_NOSCALE
            ? 1
            : 1.0 / (windowSize * windowSize)
        ...
        for (y = 0; y < h; ++y) {
          ...
          for (
            i = (srcIndex + 1) | 0, end = (srcIndex + radius) | 0;
            i <= end;
            ++i
          ) {
            sum += data_u8[i]
        ...
        if (scale == 1) {
          for (y = 0; y < w; ++y) {
            ...
            for (; x < h - radiusPlus2; x += 2, dstIndex += w2) {
              ...
              sum +=
                data_i32[nextPixelIndex + 1] - data_i32[previousPixelIndex + 1]
      ...
      gaussian_blur: function (src, dst, kernel_size, sigma) {
        ...
        kernel_size =
          kernel_size == 0
            ? (Math.max(1, 4.0 * sigma + 1.0 - 1e-8) * 2 + 1) | 0
            : kernel_size
...
;(function (global) {
  ...
  var fast_corners = (function () {
    var offsets16 = new Int32Array([
      0, 3, 1, 3, 2, 2, 3, 1, 3, 0, 3, -1, 2, -2, 1, -3, 0, -3, -1, -3, -2, -2,
      -3, -1, -3, 0, -3, 1, -2, 2, -1, 3,
    ])
        ...
        for (i = sy; i < ey; ++i) {
          ...
          if (i < ey - 1) {
            ...
            for (; j < ex; ++j, ++ptr) {
              ...
              tab = -v + 255
              d =
                thresh_tab[tab + img[ptr + pixel0]] |
                thresh_tab[tab + img[ptr + pixel8]]
              ...
              d &=
                thresh_tab[tab + img[ptr + pixel2]] |
                thresh_tab[tab + img[ptr + pixel10]]
              d &=
                thresh_tab[tab + img[ptr + pixel4]] |
                thresh_tab[tab + img[ptr + pixel12]]
              d &=
                thresh_tab[tab + img[ptr + pixel6]] |
                thresh_tab[tab + img[ptr + pixel14]]
              ...
              d &=
                thresh_tab[tab + img[ptr + pixel1]] |
                thresh_tab[tab + img[ptr + pixel9]]
              d &=
                thresh_tab[tab + img[ptr + pixel3]] |
                thresh_tab[tab + img[ptr + pixel11]]
              d &=
                thresh_tab[tab + img[ptr + pixel5]] |
                thresh_tab[tab + img[ptr + pixel13]]
              d &=
                thresh_tab[tab + img[ptr + pixel7]] |
                thresh_tab[tab + img[ptr + pixel15]]
...
;(function (global) {
  ...
  var yape06 = (function () {
    var compute_laplacian = function (
      src,
      dst,
      w,
      h,
    ...
    var hessian_min_eigen_value = function (src, off, tr, Dxx, Dyy, Dxy, Dyx) {
      ...
      var Ixy =
        src[off + Dxy] + src[off - Dxy] - src[off + Dyx] - src[off - Dyx]
        ...
        for (y = sy; y < ey; ++y, row += w) {
          for (x = sx, rowx = row; x < ex; ++x, ++rowx) {
            ...
            if (
              ...
              min_eigen_value = hessian_min_eigen_value(
                srd_d,
                rowx,
                lv,
                Dxx,
...
;(function (global) {
  ...
  var yape = (function () {
    ...
    var perform_one_point = function (
      I,
      x,
      Scores,
      Im,
    ...
    return {
      ...
      init: function (width, height, radius, pyramid_levels) {
        ...
        for (i = 0; i < pyramid_levels; ++i) {
          this.level_tables[i] = new lev_table_t(
            width >> i,
            height >> i,
            radius,
          )
        ...
        for (y = sy; y < ey; ++y, row += w) {
          for (x = sx, rowx = row; x < ex; ++x, ++rowx) {
            ...
            if (abs_score < 5) {
              ...
              if (
                third_check(scores, rowx, w) >= 3 &&
                is_local_maxima(scores, rowx, score, hw, R)
              ) {
                pt = points[number_of_points]
...
;(function (global) {
  ...
  var orb = (function () {
    var bit_pattern_31_ = new Int32Array([
      8, -3, 9, 5 /*mean (0), correlation (0)*/, 4, 2, 7,
  ...
  var optical_flow_lk = (function () {
    ...
    return {
      track: function (
        ...
        var deriv_m = new jsfeat.matrix_t(
          w0,
          h0,
          jsfeat.S32C2_t,
          deriv_lev_node.data,
        ...
        for (; level >= 0; --level) {
          ...
          for (ptid = 0; ptid < count; ++ptid) {
            ...
            min_eig =
              (A22 +
                A11 -
                Math.sqrt((A11 - A22) * (A11 - A22) + 4.0 * A12 * A12)) /
              win_area2
  ...
  var haar = (function () {
    ...
    return {
                  ...
                  for (k = 0; k < fn; ++k) {
                    ...
                    tree_sum +=
                      (int_tilted[fi_a] -
                        ...
                        int_tilted[fi_a + fw - fh + fi_b + fi_c]) *
                      feature[4]
                  }
                } else {
                  for (k = 0; k < fn; ++k) {
                    ...
                    tree_sum +=
                      (int_sum[fi_a] -
                        int_sum[fi_a + fw] -
                        int_sum[fi_a + fi_c] +
                        int_sum[fi_a + fi_c + fw]) *
                      feature[4]
                  }
                }
                stage_sum +=
                  tree_sum * inv_area < tree.threshold * std
                    ? tree.left_val
                    : tree.right_val
              }
      ...
      group_rectangles: function (rects, min_neighbors) {
        ...
        var node = []
        for (i = 0; i < n; ++i) {
          node[i] = { parent: -1, element: rects[i], rank: 0 }
        }
        for (i = 0; i < n; ++i) {
    ...
    return {
      ...
      prepare_cascade: function (cascade) {
        ...
        for (var j = 0; j < sn; j++) {
          ...
          var feature = (cascade.stage_classifier[j]._feature = new Array(
            f_cnt,
          ))
      ...
      build_pyramid: function (src, min_width, min_height, interval) {
        ...
        this.scale_to =
          (Math.log(Math.min(sw / min_width, sh / min_height)) /
            Math.log(this.scale)) |
          0
          ...
          for (j = 0; j < sn; j++) {
            ...
            for (k = 0; k < f_cnt; k++) {
              ...
              for (q = 0; q < q_cnt; q++) {
                feature_k.px[q] =
                  feature_o.px[q] * bpp +
                  feature_o.py[q] * step[feature_o.pz[q]]
                feature_k.pz[q] = feature_o.pz[q]
                feature_k.nx[q] =
                  feature_o.nx[q] * bpp +
                  feature_o.ny[q] * step[feature_o.nz[q]]
